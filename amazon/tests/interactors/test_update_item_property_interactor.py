...
class TestUpdateItemPropertyInteractor:
    ...
    def test_if_item_property_does_not_exist(self):
        ...
        with pytest.raises(NotFound):
            self.interactor.update_item_property(item_property_id=item_property_id, value=value)
    ...
    def test_update_item_property(self):
        ...
        self.storage.update_item_property.return_value = None
        self.presenter.get_response_for_update_item_property.return_value = None

        self.interactor.update_item_property(item_property_id=item_property_id, value=value)

        self.storage.check_if_item_property_exists.assert_called_once_with(item_property_id=item_property_id)
        self.storage.update_item_property.assert_called_once_with(item_property_id=item_property_id, value=value)
        self.presenter.get_response_for_update_item_property.assert_called_once()