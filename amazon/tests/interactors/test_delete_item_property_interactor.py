...
from amazon.exceptions.item_custom_exceptions import ItemPropertyDoesNotExistException
...
class TestDeleteItemPropertyInteractor:
    ...
    def test_if_item_property_does_not_exist_raises_exception(self):
        ...
        self.item_storage.check_if_item_property_exists.side_effect = ItemPropertyDoesNotExistException(item_property_id=item_property_id)
        item_presenter.raise_exception_for_item_property_does_not_exist.side_effect = NotFound

        with pytest.raises(NotFound):
            self.interactor.delete_item_property(item_property_id=item_property_id, item_presenter=item_presenter)
    ...
    def test_delete_item_property(self):
        ...
        self.item_storage.delete_item_property.return_value = None
        item_presenter.get_response_for_delete_item_property.return_value = None

        self.interactor.delete_item_property(item_property_id=item_property_id, item_presenter=item_presenter)
        
        self.item_storage.check_if_item_property_exists.assert_called_once_with(item_property_id=item_property_id)
        self.item_storage.delete_item_property.assert_called_once_with(item_property_id=item_property_id)
        item_presenter.get_response_for_delete_item_property.assert_called_once()