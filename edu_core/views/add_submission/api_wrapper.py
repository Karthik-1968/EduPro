...
def api_wrapper(*args, **kwargs):
    ...
    try:
        ...
        submitted_at=kwargs['request_data']['submitted_at']
        user=kwargs['user']
        try:
        ...
        try:
            ...
            return JsonResponse({"Error":"Student doesn't exist"},status=404)
        add_submission(assignment,student,submitted_at,user)
    except KeyError as e:
        return JsonResponse({"error": f"Missing parameter: {e}"}, status=400)
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
def add_submission(assignment,student,submitted_at,user):
    if user.email==student.email:
        ...
        Submission.objects.create(assignment=assignment,student=student,submitted_at=submitted_at)
        return JsonResponse({"Success":"Successfully submitted"},status=200)
    else:
