...
class ListofTeachers:
    ...
    def list_of_teachers(self,limit:int,offset:int,search:int,presenter:PresenterInterface):
        ...
        if search:
            ...
            teacher=self.storage.get_teacher_details(id=search)
            return presenter.get_teacher_details_response(teacher)
        else:
            teachers=self.storage.get_list_of_teachers(limit=limit,offset=offset)
            return presenter.get_list_of_teacher_details_response(teachers)